...
def save_report(args, _q_results, _file, tasks_processed_count):
    ...
    output_file_suffix = a_template['suffix']#后缀
    
    report_name = '%s_%s%s' % (os.path.basename(_file).lower().replace('.txt', ''),#去掉后缀纯文件名
    ...
    try:
        while not config.stop_me or _q_results.qsize() > 0:#stop_me=false qsize队列的大小
        ...
        if config.ports_saved_to_file:#false
